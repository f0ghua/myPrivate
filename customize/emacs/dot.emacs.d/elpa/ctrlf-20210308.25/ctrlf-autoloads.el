...

;;;### (autoloads nil "ctrlf" "ctrlf.el" (24665 27818 588057 331000))
;;; Generated autoloads from ctrlf.el

(defvar ctrlf-mode-bindings '(([remap isearch-forward] . ctrlf-forward-literal) ([remap isearch-backward] . ctrlf-backward-literal) ([remap isearch-forward-regexp] . ctrlf-forward-regexp) ([remap isearch-backward-regexp] . ctrlf-backward-regexp) ([remap isearch-forward-symbol] . ctrlf-forward-symbol) ([remap isearch-forward-symbol-at-point] . ctrlf-forward-symbol-at-point)) "\
...
See `ctrlf-mode-bindings' to customize." :keymap ctrlf--keymap (require (quote map)) (when ctrlf-local-mode (setcdr ctrlf--keymap nil) (map-apply (lambda (key cmd) (when (stringp key) (setq key (kbd key))) (define-key ctrlf--keymap key cmd)) ctrlf-mode-bindings)) (with-eval-after-load (quote ctrlf) (if ctrlf-local-mode (advice-add (function minibuffer-message) :around (function ctrlf--minibuffer-message-condense)) (advice-remove (function minibuffer-message) (function ctrlf--minibuffer-message-condense)))))
