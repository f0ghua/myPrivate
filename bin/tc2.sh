...
# 
#用最简单的话的讲,就是采用上行和下行不对等带宽的基于ATM的技术.  举例,我
#们最快的其中一条ADSL带宽是下行3200Kbit,上行只有320Kbit.带宽通常用bit表
#示.
#
#1、下行3200K 意味着什么？
# 
#因为 1Byte=8Bit ,一个字节由8个位(bit)组成,一般用大写B表示Byte,小写b表
#示Bit.  所以 3200K=3200Kbps=3200K bits/s=400K bytes/s.
# 
#2、 上行320K 意味着什么？
# 
# 320K=320Kbps=320K bits/s=40K bytes/s.
# 
#就是说,个人所能独享的最大下载和上传速度,整条线路在没任何损耗,最理想的
#时候, 下载只有400K bytes/s,上传只有最大40K bytes/s的上传网速.
# 
#这些都是理想值,但现实总是残酷的,永远没有理想中那么好.至少也有损耗,何况
#内部网有几十台电脑一起疯狂上网.
#
#3.ADSL上传速度对下载的影响
# 
#(1)TCP/IP协议规定，每一个封包，都需要有acknowledge讯息的回传，也就是说，
#传输的资料，需要有一个收到资料的讯息回复，才能决定后面的传输速度，并决
#定是否重新传输遗失的资料。上行的带宽一部分就是用来传输这些
#acknowledge(确认)资料当数据流量大的时候，就会影响acknowledge资料的传送
#速度，并进而影响到下载速度。这对非对称数字环路也就是ADSL这种上行带宽远
#小于下载带宽的连接来说影响尤为明显。
# 
#(2)试验证明，当上传满载时，下载速度变为原来速度的40％，甚至更低.因为上
#载文件(包括ftp 上传,发邮件smtp),如果较大,一个人的通讯量已经令整条adsl
#变得趋向饱和,那么所有的数据包只有按照先进先出的原则进行排队和等待.这就
#可以解释为什么网内其中有人用ftp上载文件, 或发送大邮件的时候,整个网速变
#得很慢的原因。
#
#二.解决ADSL速度之道
# 
#1. 为解决这些速度问题,我们按照数据流和adsl的特点,对经过线路的数据进行
#了有规则的分流.  把本来在adsl modem上的瓶颈转移到我们linux路由器上,可
#以把带宽控制的比adsl modem上的小一点, 这样我们就可以方便的用tc技术对经
#过的数据进行分流和控制.
# 
#我们的想象就象马路上的车道一样,有高速道,还有小车道,大车道.需要高速的
#syn,ack,icmp等走高速道,需要大量传输的ftp-data,smtp等走大车道,不能让它
#堵塞整条马路.各行其道.
# 
#2. linux下的TC(Traffic Control)就有这样的作用.只要控制得当,一定会有明
#显的效果.
# 
#tc和iptables结合是最好的简单运用的结合方法.
# 
#我们设置过滤器以便用iptables对数据包进行分类,因为iptables更灵活，而且
#你还可以为每个规则设置计数器. iptables用mangle链来mark数据包,告诉了内
#核，数据包会有一个特定的FWMARK标记值(hanlde x fw)，表明它应该送给哪个
#类( classid x : x),而prio是优先值,表明哪些重要数据应该优先通过哪个通道.
# 
#首先选择队列,cbq和htb是不错的选择,经过实验,htb更为好用,所以以下脚本采
#用htb来处理.
# 
#3. 一般系统默认的是fifo的先进先出队列,就是说数据包按照先来先处理的原
#则,如果有一个大的数据包在前面,#那么后面的包只能等前面的发完后才能接着
#发了,这样就算后面即使是一个小小的ack包, 也要等待了,这样上传就影响了下
#载,就算你有很大的下载带宽也无能为力.
# 
...
# 
#HTB就象CBQ一样工作，但是并不靠计算闲置时间来整形。它是一个分类的令牌桶
#过滤器。它只有很少的参数.
# 
#他的分层(Hierarchical)能够很好地满足这样一种情况：你有一个固定速率的链
#路，希望分割给多种不同的用途使用,为每种用途做出带宽承诺并实现定量的带
#宽借用。
# 
#4. 结构简图:
# 
...
#关于参数的说明
# 
#(1)rate: 是一个类保证得到的带宽值.如果有不只一个类,请保证所有子类总和
#         是小于或等于父类.
# 
#(2)ceil: ceil是一个类最大能得到的带宽值.
# 
#(3)prio: 是优先权的设置,数值越大,优先权越小.如果是分配剩余带宽,就是数
#         值小的会最优先取得剩余的空闲的带宽权.
# 
#具体每个类要分配多少rate,要根据实际使用测试得出结果.
# 
#一般大数据的话,控制在50%-80%左右吧,而ceil最大建议不超过85%,以免某一个
#会话占用过多的带宽.
# 
#rate可按各类所需分配,
# 
#1:11 是很小而且最重要的数据包通道,当然要分多点.甚至必要时先全部占用,不
#过一般不会的.所以给全速.
# 
#1:12 是很重要的数据道,给多点,最少给一半,但需要时可以再多一点.
#rate 规划 1:2 = 1:21 + 1:22 + 1:23 + 1:24 一般总数在50%-80%左右
# 
#1:21 http,pop是最常用的啦,为了太多人用,而导致堵塞,我们不能给得太多,也
#不能太少.
# 
#1:22 我打算给smtp用,优先低于1:21 以防发大的附件大量占用带宽
# 
#1:23 我打算给ftp-data,和1:22一样,很可能大量上传文件,所以rate不能给得太
#多,而当其他有剩时可以给大些,ceil设置大些
# 
#1:24 是无所谓通道,就是一般不是我们平时工作上需要的通道了,给小点,防止这
#些人在妨碍有正常工作需要的人
# 
...
#1.2.2 在次干类下建立第二叶子类。不要太高的速度,以防发大的附件大量占用
#      带宽,例如smtp等
tc class add dev $DEV parent 1:2 classid 1:22 htb rate 30kbit ceil $[$UPLINK-160]kbit prio 5

#1.2.3 在次干类下建立第三叶子类。不要太多的带宽,以防大量的数据堵塞网络,例
#      如ftp-data等,
tc class add dev $DEV parent 1:2 classid 1:23 htb rate 15kbit ceil $[$UPLINK-170]kbit prio 6

#1.2.4 在次干类下建立第四叶子类。无所谓的数据通道,无需要太多的带宽,以防
#      无所谓的人在阻碍正务.
tc class add dev $DEV parent 1:2 classid 1:24 htb rate 5kbit ceil $[$UPLINK-250]kbit prio 7

#在每个类下面再附加上另一个队列规定,随机公平队列(SFQ)，不被某个连接不停
#占用带宽,以保证带宽的平均公平使用：
# 
#SFQ(Stochastic Fairness Queueing，随机公平队列),SFQ的关键词是"会话"(或
#称作"流") ，主要针对一个TCP会话或者UDP流。流量被分成相当多数量的FIFO队
#列中，每个队列对应一个会话。数据按照简单轮转的方式发送, 每个会话都按顺
#序得到发送机会。这种方式非常公平，保证了每一个会话都不会没其它会话所淹
#没。SFQ之所以被称为"随机"，是因为它并不是真的为每一个会话创建一个队列，
#而是使用一个散列算法，把所有的会话映射到有限的几个队列中去。参数
#perturb是多少秒后重新配置一次散列算法。默认为10
...
#这里设置过滤器,handle 是iptables作mark的值,让被iptables 在mangle链做了
#mark的不同的值选择不同的通道classid,而prio 是过滤器的优先级别.
...
#6. 下行的限制:
# 
#设置入队的规则,是因为把一些经常会造成下载大文件的端口进行控制,不让它们
#来得太快,导致堵塞.来得太快的就直接drop,就不会浪费和占用机器时间和力量
#去处理了.
# 
#(1).把下行速率控制在大概1000-1500k左右,因为这个速度已经足够用了,以便
#    能够得到更多的并发下载连接
...
#(2).如果内部网数据流不是很疯狂的话,就不用做下载的限制了,用#符号屏蔽上
#    面两行即可.
# 
